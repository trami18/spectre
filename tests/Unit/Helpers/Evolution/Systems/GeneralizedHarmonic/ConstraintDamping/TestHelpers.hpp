...
void check_impl(
  ...
  const auto helper =
      [&python_function_prefix, &random_value_bounds, &member_args_tuple,
       &function_of_time_name, &used_for_size](
        ...
        // respectively, \f$a_0,a_1,2 a_2, 6_a3\f$.
        //
        ...
        const auto damping_function_call_operator_helper =
            [&gh_damping_function, &random_value_bounds,
             &function_of_time_name](
              ...
              // function_of_time to the lower bound in random_value_bounds.
 ...
 * call operator. For time-dependent FunctionsOfTime, this parameter must be
 * consistent with the FunctionOfTime name that in_gh_damping_function's call
 * operator expects. For time-independent FunctionsOfTime,
...
void check(std::unique_ptr<DampingFunctionType> in_gh_damping_function,
  ...
  detail::check_impl(
      ...
      function_of_time_name, member_args...);
}
